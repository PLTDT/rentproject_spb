...
//			doc = EcpayFunction.xmlParser(configPath);
			...
			ClassLoader classLoader = Thread.currentThread().getContextClassLoader();//取得目前專案在伺服器上執行的資源
			URL resourcesUrl = classLoader.getResource("payment_conf.xml");
			if (resourcesUrl != null) {
				String configPath = URLDecoder.decode(resourcesUrl.getPath(), "UTF-8");
				doc = EcpayFunction.xmlParser(configPath);
			} else {
